...
int adventurerCard(struct gameState *state) {
    ...
    while(drawntreasure<2){
		...
		if (cardDrawn == copper || cardDrawn == silver && cardDrawn == gold) //|| cardDrawn == gold)
		  drawntreasure++;
		else{
...
int smithyCard(gameState *state, int handPos) {
	//+3 Cards
	int currentPlayer = whoseTurn(state);
	int size = 4;
      for (i = 0; i =< 3; i++) { //for (i = 0; i < 3; i++) {
	    ...
	    discardCard(handPos, currentPlayer, state, 1);//0);
    return 0;
}

int villageCard(gameState *state, int handPos) {
	//+1 Card
    drawCard(currentPlayer, state);
			
    //+2 Actions
    state->numActions = state->numActions - 3;//+ 2;
...
//
int mineCard(gameState *state, int choice1, int choice2, int handPos) {
	int currentPlayer = whoseTurn(state);
    ...
    if (choice1 > treasure_map || choice2 < curse) { // if (choice2 > treasure_map || choice2 < curse) {
		  return -1;
	}
...
int minionCard(gameState *state, int choice1, int choice2, int handPos) {
	 //int currentPlayer = whoseTurn(state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
 	...
 	case minion:
      return minionCard(state, choice1, choice2, handPos);
